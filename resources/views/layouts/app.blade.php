            ...
            <div class="container">
                <div class="d-flex justify-content-between  align-items-center">
                    <ul class="d-flex icone_rede_social_site">
                        <li class="ps-4"><a class="icone_rede_social_site link-success text-white fs-5"
                                href="{{ route('saiba_mais') }}">Saiba Mais</a></li>
                        <li class="ps-4"><a class="icone_rede_social_site link-success text-white fs-5"
                                href="{{ route('quem_somos') }}">Quem Somos</a></li>
                        <li class="ps-4"><a class="icone_rede_social_site link-success text-white fs-5"
                                href="{{ route('fale_conosco') }}">Fale conosco</a></li>
                    </ul>
  ...
  <button class="btn btn-default btn-sm" onclick="removerItemDoCarrinho(${propriedade});"><i class="fa-solid fa-trash"></i></button>
            ...
            function removerItemDoCarrinho(indice) {
                if (confirm("Você realmente deseja remover esse item do carrinho?")) {
                    ...
                    if (item) {
                        let botaoItem = document.querySelector(`button[data-id="${item.id}"]`);

                        if (botaoItem != null) {
                            botaoItem.innerHTML = `Adicionar ao carrinho`;
                            botaoItem.classList.remove("disabled");
                        }
                    }
            ...
            function preencherSelecionados(carrinho) {
                for (let item of carrinho) {
                    let botaoItem = document.querySelector(`button[data-id="${item.id}"]`);
                    console.log(item, botaoItem);
                    if (botaoItem != null) {
                        botaoItem.innerHTML = "Remover do carrinho";
                        botaoItem.classList.add("disabled");
                    }
                }
            }
            carregarCarrinho();
